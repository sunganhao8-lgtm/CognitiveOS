...
>
> **不可替代的核心机制：主动复现校验。** Agent 的"记忆"只是**被动记录**——"主人说过 X"——遇到类似问题还是会踩同样的雷。CognitiveOS 把主人铁律作为**测试用例**，每次新会话 / 切换 Agent / 升级提示词时主动跑一遍，**校验 Agent 是否还踩同样的雷**。这件事 Agent 自己干不了：它自己看不到自己、不会主动回放过去的纠正、不会把规则变成可执行的探针。
>
> CognitiveOS 做三件事，每一件都站在 Agent 外面：
>
> 1. **主动观察**：监控 Agent 对主人的应答，识别"主人纠正过 / 偏好过 / 骂过"的模式
> 2. **主动提炼**：把零散反馈结构化成铁律（领域禁忌、决策风格、措辞偏好），写入主人档案
> 3. **主动复现校验**：把铁律当测试 case 跑给新会话/新 Agent/新 prompt 看是否仍踩雷
>
> ——导入导出做得到第一件的一半；剩下两件，Agent 工程自己干不了。
